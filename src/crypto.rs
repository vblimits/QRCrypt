...
impl Crypto {
    ...
    pub fn encrypt(&self, data: &SecretData, password: &str) -> Result<EncryptedData> {
        ...
        let hash = password_hash.hash.ok_or_else(|| {
            QRCryptError::Encryption("Failed to extract key from hash".to_string())
        })?;
    ...
    pub fn decrypt(&self, encrypted: &EncryptedData, password: &str) -> Result<SecretData> {
        if encrypted.version != 1 {
            return Err(QRCryptError::Decryption(format!(
                "Unsupported version: {}",
                encrypted.version
            )));
        }
        ...
        let hash = password_hash.hash.ok_or_else(|| {
            QRCryptError::Decryption("Failed to extract key from hash".to_string())
        })?;
        let key_bytes = hash.as_bytes();

        if key_bytes.len() < 32 {
            return Err(QRCryptError::Decryption(
                "Derived key too short".to_string(),
            ));
        }
    ...
    pub fn encrypt_with_decoy(
        &self,
        real_data: &SecretData,
        real_password: &str,
        decoy_data: &SecretData,
        decoy_password: &str,
        decoy_hint: Option<String>,
    ) -> Result<LayeredData> {
    ...
    pub fn decrypt_layered(
        ...
        Err(QRCryptError::Decryption(
            "Password does not match any layer".to_string(),
        ))
    }
}
